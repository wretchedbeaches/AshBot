...
export class pokemon_stats extends Model<pokemon_statsAttributes, pokemon_statsCreationAttributes> implements pokemon_statsAttributes {
  date!: string;
  pokemon_id!: number;
  count!: number;


  static initModel(sequelize: Sequelize.Sequelize): typeof pokemon_stats {
    pokemon_stats.init({
    date: {
