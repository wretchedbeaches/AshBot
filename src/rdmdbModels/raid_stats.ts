...
export class raid_stats extends Model<raid_statsAttributes, raid_statsCreationAttributes> implements raid_statsAttributes {
  date!: string;
  pokemon_id!: number;
  count!: number;
  level?: number;
  ...
  static initModel(sequelize: Sequelize.Sequelize): typeof raid_stats {
    raid_stats.init({
    date: {
