...
export class invasion_stats
	extends Model<invasion_statsAttributes, invasion_statsCreationAttributes>
	implements invasion_statsAttributes
{
	public date!: string;
	public grunt_type!: number;
	public count!: number;

	public static initModel(sequelize: Sequelize.Sequelize): typeof invasion_stats {
		invasion_stats.init(
			{
				date: {
				...
				indexes: [
					{
						...
						unique: true,
						using: 'BTREE',
						fields: [{ name: 'date' }, { name: 'grunt_type' }],
					},
				],
			},
		);
