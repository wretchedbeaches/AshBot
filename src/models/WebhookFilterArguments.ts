...
// 		if (names.every((name) => Object.values(masterfile.pokemon).some((pokemon) => pokemon.name === name))) return names;
// 		return null;
// 	},
// 	flag: ['name'],
// 	prompt: {
...
// 		try {
// 			if (Object.keys(config.cities).includes(str.toLowerCase())) return { city: str };
// 			const parts = str.split(' ');
// 			const unit = parts[0].endsWith('km') ? 'km' : parts[0].endsWith('m') ? 'm' : null;
// 			if (unit === null) return null;
// 			return {
// 				center: [Number(parts[1].split(',')[0]), Number(parts[1].split(',')[1])],
// 				radius:
// 					unit === 'km'
...
// 	prompt: {
// 		optional: true,
// 		start: (msg: Message) => `${msg.author}, please provide a number between 0 and 100. **Example: miniv 50**`,
// 		retry: (msg: Message) => `${msg.author}, please provide a number between 0 and 100. **Example: miniv 50**`,
// 	},
