...
export function parsePokemon(
  ...
  embed.setDescription(
    ...
    `${pokemonData ? '**' + pokemonData.name + '**' : ''} ${
      ...
      // line 3: cp, iv, and lvl
      (cp !== null && iv !== null && pokemon_level !== null
        ? `${emoji(emoji(config.statsEmojis.cp))} **${cp}**\n ${emoji(
      ...
      `${emoji(emoji(config.statsEmojis.despawn))}: ${duration}\n` +    
      ...
      ${emoji(
        config.statsEmojis.google)}[[**Google**](https://www.google.com/maps?q=${latitude},${longitude})] ${emoji(
        config.statsEmojis.apple
      )} [[**Apple**](http://maps.apple.com/maps?daddr=${latitude},${longitude}&z=10&t=s&dirflg=d)]
      `
  );
  return { embed: embed, message: `${emoji('pokemon_' + pokemon_id)} ${pokemonData.name} | ${latitude.toFixed(5)},${longitude.toFixed(5)}` };
...
export function parseQuestDb(quest): Object {
  ...
  const emoji = countryFlagEmoji.get(city.country);
  return {
    value: stripIndents`**Type:** ${masterfile.quest_types[quest_type + ''].type
      .split('{0}')
      .join(quest_target)}
...
export function parseQuest(
  ...
  embed.setURL(`https://www.google.com/maps?q=${latitude},${longitude})`);
   switch(rewards[0].type) {
     case 2:
      embed.setThumbnail(`https://raw.githubusercontent.com/nileplumb/PkmnShuffleMap/master/PMSF_icons_large/rewards/reward_${quest.rewards[0].info.item_id}_1.png`);
     break;
     case 3:
      embed.setThumbnail(`https://raw.githubusercontent.com/nileplumb/PkmnShuffleMap/master/PMSF_icons_large/rewards/reward_stardust_${quest.rewards[0].info.amount}.png`);
     break; 
     case 7: 
     if (quest.rewards[0].info.pokemon_id) embed.setThumbnail("https://raw.githubusercontent.com/nileplumb/PkmnHomeIcons/master/RDM_OS_128/pokemon/" + quest.rewards[0].info.pokemon_id + ".png");
     break;
     case 12: 
     if (quest.rewards[0].info.pokemon_id) embed.setThumbnail("https://raw.githubusercontent.com/nileplumb/PkmnShuffleMap/master/PMSF_icons_large/rewards/reward_mega_energy_" + quest.rewards[0].info.pokemon_id + ".png");
     break;
   };
   ...
   // CATCHING SPECIFIC POKEMON
    ...
    case quest.template.indexOf('catch') >= 0:
      if (quest.conditions && quest.conditions[0]) {
        if (
          quest.conditions[0].info &&
          quest.conditions[0].info.pokemon_type_ids[0]
        ) {
          return (
            'Catch ' +
            quest.target +
    ...
    default:
      console.error(
        'UPDATE FOR THIS QUEST (' + quest.pokestop_id + ')',
        quest
      );
